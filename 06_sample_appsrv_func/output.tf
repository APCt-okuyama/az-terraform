...
output "output-message1" {
  ...
  # deploy コマンドの例
  value       = "(app service): az webapp up --runtime PYTHON:3.9 -n ${local.appservice_name} -g ${local.rg_name} -p ${local.appservice_plan_name} -l ${local.location}"
}
...
output "output-message3" {
  ...
  # deploy コマンドの例 az storage blob upload-batch -s dist -d '$web' --account-name examplestorageacc6650 --overwrite
